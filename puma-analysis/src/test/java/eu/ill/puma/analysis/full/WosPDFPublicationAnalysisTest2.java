...
@Ignore
public class WosPDFPublicationAnalysisTest2 extends FullAnalysisTest {
	...
	public void importDocument() throws Exception {
		...
		DocumentVersion documentVersion = this.getDocumentVersion("wos", "private/importers/wos/PDFPublicationDocument2.json", "private/importers/wos/ImporterInfo.json");
		...
		Assert.assertEquals(3, this.entityOriginService.getAllForDocumentVersionAndEntityType(documentVersion, EntityType.RESEARCH_DOMAIN).size());
		...
		Assert.assertEquals(0, this.entityOriginService.getAllForDocumentVersionAndEntityType(documentVersion, EntityType.PERSON).size());
		...
		Assert.assertEquals(24, allOrigins.size());
	}
