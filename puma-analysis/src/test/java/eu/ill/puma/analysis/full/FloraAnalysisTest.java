...
@Ignore
public class FloraAnalysisTest extends FullAnalysisTest {
	...
	public void testImportDocument1() throws Exception {
		...
		DocumentVersion documentVersion = this.getDocumentVersion("flora", "private/importers/flora/FloraDocument1.json", "private/importers/flora/ImporterInfo.json");
		...
		Assert.assertEquals(1, this.entityOriginService.getAllForDocumentVersionAndEntityType(documentVersion, EntityType.RESEARCH_DOMAIN).size());
		Assert.assertEquals(1, this.entityOriginService.getAllForDocumentVersionAndEntityType(documentVersion, EntityType.LABORATORY).size());
		Assert.assertEquals(8, this.entityOriginService.getAllForDocumentVersionAndEntityType(documentVersion, EntityType.PERSON).size());
		...
		Assert.assertEquals(16, allOrigins.size());
	}
